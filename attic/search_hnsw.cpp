...
int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "must input the path of image: search_hnsw lena.jpg"
              << std::endl;
    return -1;
  }
  ...
  std::unique_ptr<hnswlib::HierarchicalNSW<float>> alg_hnsw(
      new hnswlib::HierarchicalNSW<float>(&space, hnsw_path));
  ...
  std::priority_queue<std::pair<float, hnswlib::labeltype>> result =
      alg_hnsw->searchKnn(buf.data(), numNeighbors);

  while (!result.empty()) {
    auto label = result.top();
    std::cout << "label: " << label.second << ", distance: " << label.first
              << std::endl;
    result.pop();
  }
}
